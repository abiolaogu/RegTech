# Deployment Guide — RegTech
> Version: 1.0 | Last Updated: 2026-02-18 | Status: Draft
> Classification: Internal | Author: AIDD System

## 1. Overview

Deployment procedures for RegTech across all environments.
...
```bash
kubectl create namespace regtech
kubectl label namespace regtech istio-injection=enabled
```

### 3.3 Secrets Configuration
```bash
# Create secrets from Vault
vault kv get -format=json secret/regtech/db | \
...
docker build -t registry.billyrinks.com/regtech:$VERSION .
docker push registry.billyrinks.com/regtech:$VERSION

# Step 2: Deploy to green environment
helm upgrade --install regtech ./charts \
  --namespace regtech \
...
kubectl patch service regtech -p '{"spec":{"selector":{"version":"green"}}}'
```
...
kubectl patch service regtech -p '{"spec":{"selector":{"version":"blue"}}}'

# Or rollback Helm release
helm rollback regtech --namespace regtech
```
...
```bash
# Check pod status
kubectl get pods -n regtech

# Check service health
curl https://api.billyrinks.com/v1/health

# Verify metrics
curl http://prometheus:9090/api/v1/query?query=up{job="regtech"}
```
